...
def get_scale(mesh_path,
    ...
    radius = 2 * radius
    print(f'[{dt_datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] rendering radius', radius)

    # Render multimle images and feature matching
    print(f'[{dt_datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] rendering objects...')
    ...
    print(f'[{dt_datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] matching features...')
...
def load_data(color_img_path: str, 
    ...
    scale_matrix = np.array([[scale, 0, 0, 0],
                             ...
                             [0, 0, scale, 0],
                             [0, 0, 0, 1]])
    print(f"[{dt_datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] initial scale: {scale}")
    ...
    for i in range(3):
        ...
        mesh.apply_transform(scale_matrix)
        mesh_rotated.apply_transform(scale_matrix)
    print(f"[{dt_datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] final scale: {scales}")
    print(f"[{dt_datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] final pose: {poses}")
    final_scale = 1.0
    for scale in scales:
      final_scale *= scale
